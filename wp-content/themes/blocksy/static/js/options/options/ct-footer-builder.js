...
export const fetchCurrentFooter = () => {
	...
	if (
		wp.customize.previewer.container
			.find('iframe')[0]
			.contentDocument.querySelector('footer.site-footer')
	) {
		return wp.customize.previewer.container
			.find('iframe')[0]
			.contentDocument.querySelector('footer.site-footer').dataset.id
	}
...
const FooterBuilder = ({
	...
	if (currentFooter.current === null) {
		currentFooter.current = fetchCurrentFooter()
	}
	...
	useEffect(() => {
		...
		wp.customize('footer_placements')({
			...old,
			__forced_static_footer__: fetchCurrentFooter()
		})
