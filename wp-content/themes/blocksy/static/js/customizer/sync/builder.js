...
wp.customize.bind('preview-ready', () => {
		...
		if (this.params.loader_selector) {
			if (this.params.loader_selector.indexOf(':') > -1) {
				...
				$(placement.container)
					...
					.map(el => el.classList.add('customize-partial-refreshing'))
			} else {
				$(placement.container)
	...
	wp.customize.preview.bind(
		...
		({ optionId, optionValue, values, futureItems, itemId }) => {
			...
			ctEvents.trigger(`ct:header:sync:item:${getOriginalId(itemId)}`, {
			...
			if (!headerVariableDescriptors[getOriginalId(itemId)]) return

			const itemDescriptors =
				headerVariableDescriptors[getOriginalId(itemId)]
