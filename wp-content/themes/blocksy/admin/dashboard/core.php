...
class Blocksy_Dashboard_Page {
	...
	public function load_system_status() {
		$system_status = new Blocksy_System_Status();
		wp_send_json_success($system_status->output());
	}
	...
	public function enqueue_static() {
		...
		wp_localize_script(
			...
			[
				...
				'is_companion_active' => $manager->get_companion_status()['status'] === 'active' ? 'yes' : 'no',
		...
		if (is_rtl()) {
			wp_enqueue_style(
				...
				get_template_directory_uri() . '/admin/dashboard/static/bundle/main-rtl.css',
				['ct-dashboard-styles'],
				$theme->get('Version')
			);
		}
