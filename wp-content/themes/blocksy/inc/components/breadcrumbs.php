...
class Blocksy_Breadcrumbs_Builder {
	...
	private function build_breadcrumbs() {
		...
		if (get_theme_mod('breadcrumb_home_item', 'text') === 'icon') {
			$home_icon = '<svg width="15" viewBox="0 0 24 20"><path d="M12,0L0.4,10.5h3.2V20h6.3v-6.3h4.2V20h6.3v-9.5h3.2L12,0z"/></svg>';
		}
	...
	public function render() {
		...
		?>
			...
			<ul class="ct-breadcrumbs" <?php echo blocksy_schema_org_definitions('breadcrumb_list') ?>>
				<?php for ( $i = 0; $i < count( $items ); $i ++ ) : ?>
					<?php if ( $i == ( count( $items ) - 1 ) ) : ?>
						<li class="last-item" <?php echo blocksy_schema_org_definitions('breadcrumb_item') ?>>
							<?php if (isset($items[$i]['icon'])) { ?>
								<?php echo $items[$i]['icon']; ?>
							<?php } ?>

							<span <?php echo blocksy_schema_org_definitions('name') ?>>
					...
					<?php elseif ( $i == 0 ) : ?>
						...
						<li class="first-item" <?php echo blocksy_schema_org_definitions('breadcrumb_item') ?>>

							<?php if (isset($items[$i]['url'])) { ?>
								<a
									href="<?php echo esc_attr( $items[ $i ]['url'] ) ?>"
									<?php echo blocksy_schema_org_definitions('url') ?>>
					...
					<?php else : ?>
						...
						<li class="<?php echo( $i - 1 ) ?>-item" <?php echo blocksy_schema_org_definitions('breadcrumb_item') ?>>
							<?php if (isset($items[$i]['url'])) { ?>
								<a
									href="<?php echo esc_attr($items[$i]['url']) ?>"
									<?php echo blocksy_schema_org_definitions('url') ?>>
