...
<div
	...
	<<?php echo $tag ?> class="site-title" <?php echo blocksy_schema_org_definitions('name') ?>>
		<?php if (! $custom_logo_id) { ?>
			<a href="<?php echo esc_url(home_url('/')); ?>" rel="home" <?php echo blocksy_schema_org_definitions('url')?>>
				<?php
					echo blocksy_translate_dynamic(blocksy_default_akg(
						'blogname',
						$atts,
						get_bloginfo('name')
					), 'header:' . $header_id . ':logo:blogname');
				?>
			</a>
		<?php } else { ?>
