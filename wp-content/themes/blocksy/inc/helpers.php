...
add_filter('body_class', function ($classes) {
	...
	$footer_render = new Blocksy_Customizer_Builder_Render_Columns();
...
if (! function_exists('blocksy_body_attr')) {
	function blocksy_body_attr() {
		...
		$attrs['data-prefix'] = blocksy_manager()->get_prefix_for_current_screen();
		// }
	...
	add_filter('pre_get_posts', function ($query) {
		if ($query->is_search && (
			...
			if (!empty($_GET['ct_post_type'])) {
				$query->set(
					'post_type',
					explode(':', sanitize_text_field($_GET['ct_post_type']))
				);
			}
		}
...
if (! function_exists('blocksy_get_with_prefix')) {
...
function blocksy_get_selector_for_prefix($prefix = '') {
	if (empty($prefix)) {
		return '';
	}

	return 'body[data-prefix="' . $prefix . '"]';
}
